...
export const Constants = {
  ...
  header: {
    selectCountry: valueLen === 'es'? 'Seleccionar pais' : 'Select country',
    selectCity: valueLen === 'es'? 'Seleccionar ciudad' : 'Select city',
  ...
  login: {
    login: valueLen === 'es'? 'Iniciar Sesión' : 'Login',
    email: valueLen === 'es'? 'Correo electronico' : 'Email',
    pass: valueLen === 'es'? 'Contraseña' : 'Password',
    session: valueLen === 'es'? 'Mantener sesión' : 'Keep session',
    enter: valueLen === 'es'? 'Ingresar' : 'Login',
    account: valueLen === 'es'? '¿No tienes una cuenta?' : "Don't have an account?",
    register: valueLen === 'es'? 'Regístrate' : 'Register',
  },
  register: {
    register: valueLen === 'es'? 'Regístrate' : 'Register',
    createAccount: valueLen === 'es'? 'Crea tu cuenta para empezar a conocer el mundo' : 'Create your account to start getting to know the world',
    name: valueLen === 'es'? 'Nombre' : 'Name',
    email: valueLen === 'es'? 'Correo electronico' : 'Email',
    pass: valueLen === 'es'? 'Contraseña' : 'Password',
    confirmPass: valueLen === 'es'? 'Confirmar contraseña' : 'Confirm password',
    country: valueLen === 'es'? 'Pais' : 'Country',
    timeZone: valueLen === 'es'? 'Zona Horaria' : 'Time zone',
    language: valueLen === 'es'? 'Idioma' : 'Language',
    obName: valueLen === 'es'? 'El nombre es obligatorio' : 'The name is required',
    obEmail: valueLen === 'es'? 'El email es obligatorio y tiene que ser valido' : 'The email address is required and must be valid.',
    obPass: valueLen === 'es'? 'La contraseña es obligatoria' : 'Password is required',
    samePass: valueLen === 'es'? 'Las contraseñas deben de ser iguales' : 'Passwords must be the same',
    obCountry: valueLen === 'es'? 'El pais es obligatorio' : 'The country is required',
    obTimeZone: valueLen === 'es'? 'La zona horaria es obligatoria' : 'The time zone is required',
    obLanguage: valueLen === 'es'? 'El idioma es obligatorio' : 'Language is required',
    create: valueLen === 'es'? 'Crear cuenta' : 'Create an account',
    account: valueLen === 'es'? '¿Ya tienes una cuenta?' : 'Already have an account?',
    login: valueLen === 'es'? 'Iniciar Sesión' : 'Login',
  },
  countries: [
  ...
  timeZones: [
    ...
    {
      value: 'am',
      timeZones: [{ name: 'Hora de Armenia', utc: 4, code: 'AMT' }],
    },
    {
      value: 'no',
      timeZones: [{ name: 'Hora Central Europea', utc: 1, code: 'CET' }],
    ...
    {
      value: 'uy',
      timeZones: [{ name: 'Hora de Uruguay', utc: -3, code: 'UYT' }],
    },
  ],
  languages: [
